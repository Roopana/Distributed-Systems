...
public class ClientHandlerServerAtCoordinator extends Thread {
	...
	public ClientHandlerServerAtCoordinator(Socket socket, Socket serverToCoordinatorSock) {
	...
	public void run() {
		...
		try {
			...
			if(input.equalsIgnoreCase(GET_ALL_SERVERS)) {
				...
				String serverIP = serverToCoordinatorSock.getInetAddress().toString() + serverToCoordinatorSock.getLocalPort();
...
//						
//					}
					System.out.println("Reading consistency type is: " + consiType);
					...
					case SEQUENTIAL_CONSISTENCY:
						serviceObj = new SequentialConService(serverToCoordinatorSock, socket.getInetAddress().toString()+socket.getPort()
								);

						break;
					case QUORUM_CONSISTENCY:
						serviceObj = new QuorumConsistService(serverToCoordinatorSock,
								socket.getInetAddress().toString()+socket.getPort(), 2, 2); 
						break;
					case READ_YOUR_WRITE_CONSISTENCY:
						serviceObj = new ReadYourWriteConService(serverToCoordinatorSock, socket.getInetAddress().toString()+socket.getPort()
								);
						break;
					case EXIT:
